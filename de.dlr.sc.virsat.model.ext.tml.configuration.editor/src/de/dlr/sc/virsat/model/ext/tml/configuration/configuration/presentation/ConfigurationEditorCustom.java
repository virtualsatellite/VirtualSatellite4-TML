...
import de.dlr.sc.virsat.model.dvlm.structural.provider.StructuralItemProviderAdapterFactory;
...
import de.dlr.sc.virsat.model.ext.tml.configuration.configuration.provider.ConfigurationItemProviderAdapterFactory;
 ...
 *
 */
public class ConfigurationEditorCustom extends ConfigurationEditor {

	Resource dmfResource = null;
	...
	protected void handleActivate() {
		...
		if (!removedResources.isEmpty()) {
			if (handleDirtyConflict()) {
				getSite().getPage().closeEditor(ConfigurationEditorCustom.this, false);
			} else {
		...
		// Set selection to DMF resource
		setSelectionToViewer(Collections.singleton(editingDomain.getWrapper(dmfResource.getContents().get(0))));
	}
	...
	public void createModel() {
		...
		if (resource.getContents().get(0) instanceof DObjectContainer) {
			this.dmfResource = resource;
		}
	...
	protected void initializeEditingDomain() {
		...
		adapterFactory.addAdapterFactory(
				new de.dlr.sc.virsat.model.ext.tml.structural.structural.provider.StructuralItemProviderAdapterFactory());
