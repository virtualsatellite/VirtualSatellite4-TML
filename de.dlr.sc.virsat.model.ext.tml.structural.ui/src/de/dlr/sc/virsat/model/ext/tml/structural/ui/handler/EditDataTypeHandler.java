...
public class EditDataTypeHandler extends AbstractHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		CategoryAssignment categorySubject = getDataTypeCAssignment(selectionHelper.getFirstEObject());
		CategoryAssignment categoryContainer = getDateTypeContainerCA(selectionHelper.getFirstEObject());
	...
	protected static void openXtextEditor(CategoryAssignment containerCatergoryAssignment,
		...
		DatatypeDefinitionEditorInput editorInput = createEditorInput(containerCatergoryAssignment,
	 ...
	 * @return editor input
	 */
	protected static DatatypeDefinitionEditorInput createEditorInput(CategoryAssignment containerCatergoryAssignment,
			CategoryAssignment subjectCatergoryAssignment) {
		...
		return new DatatypeDefinitionEditorInput(file, resourceAndEObjectUri, containerCatergoryAssignment,
				subjectCatergoryAssignment);
	}
	...
	public boolean isEnabled() {
		...
		if (selectionHelper.getFirstEObject() instanceof ComposedPropertyInstance) {
			...
			return property.getType().getName().equals(ATaskingDefinition.PROPERTY_DATATYPES) || property.getType().getName().equals(ATaskingDefinition.PROPERTY_EXTERNALTYPES);
		}
		...
		if (selectionHelper.getFirstEObject() instanceof CategoryAssignment) {
			...
			return categoryA.getType().getFullQualifiedName().equals(DataType.FULL_QUALIFIED_CATEGORY_NAME);
		}
	 ...
	 * @return the data type as category assignment
	 */
	protected CategoryAssignment getDataTypeCAssignment(EObject selection) {
	...
	/**
	 * Returns the tasking definition for a selected element
	 * @param selection selection of the current UI request
	 * @return the category assignment of the tasking environment
	 */
	protected CategoryAssignment getDateTypeContainerCA(EObject selection) {
		...
		if (selection instanceof ComposedPropertyInstance) {
			return getDateTypeContainerCA(((ComposedPropertyInstance) selection).getTypeInstance());
		}
