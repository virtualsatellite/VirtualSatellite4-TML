...
public class AddEnumerationHandler extends AbstractHandler implements IHandler {
	...
	protected static void openXtextEditor(CategoryAssignment catergoryAssignment) {
		...
		EnumDefinitionEditorInput editorInput = createEditorInput(catergoryAssignment);
		try {
	 ...
	 * @param catergoryAssignment container for new enumeration as category assignment
	 * @return editor input
	 */
	protected static EnumDefinitionEditorInput createEditorInput(CategoryAssignment catergoryAssignment) {
		...
		return new EnumDefinitionEditorInput(file, resourceAndEObjectUri, catergoryAssignment, null);
	}
	...
	public boolean isEnabled() {
		...
		if (selectionHelper.getFirstEObject() instanceof ArrayInstance) {
			...
			return arrayProperty.getType().getFullQualifiedName().equals(
					ATaskingDefinition.FULL_QUALIFIED_CATEGORY_NAME + "." + ATaskingDefinition.PROPERTY_ENUMERATIONS);
		}
