 ...
 * 
 * @author fran_tb
 * Custom class for enum editor input
...
public class EnumDefinitionEditorInput extends VirsatCategoryXtextEditorInput {
	 ...
	 */
	public EnumDefinitionEditorInput(IFile resourceFile, URI uri, CategoryAssignment container,
	...
	public String getName() {
		...
		if (getSubject() == null) {
			return "New Enumeration -" + getContainer().getName();
		} else {
	...
	public void reintegrateEditorSubject(EObject subject, String serializedData) {
		if (subject instanceof Enumeration) {
			((Enumeration) subject).setSerializedDefinition(serializedData);
			if (getSubject() != null) {
				updateSubject(getSubject(), subject);
			} else {
				getContainer().getEnumerations().add((Enumeration) subject);
				dmfSubject = subject;
			}
		}
		saveDMFResource();
	...
	protected EObject getSubjectFromDMFRoot(EObject dmfRoot, CategoryAssignment container, CategoryAssignment subject) {
		if (subject != null) {
			for (Enumeration enumeration : getContainer().getEnumerations()) {
				if (enumeration.getName().equals(subject.getName())) {
					return enumeration;
				}
			}
		}
	...
	protected EObject getInitialSubject() {
		return StructuralFactory.eINSTANCE.createEnumeration();
	}
